    ...
    try:
        ...
        if os.path.exists(config_file):
            with open(config_file, 'r', encoding='utf-8') as f:
                config = json.load(f)
                return config.get('first_login', True)
        return True  # First login if no config file
    except:
        return True
...
def save_admin_password(new_password):
    """Save new admin password to config"""
    try:
        config_file = os.path.join('config', 'web_settings.json')
        config = {}
        ...
        logging.error(f"Error saving admin password: {e}")
        return False
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Check if password change is required (except for change-password and logout routes)
        if (session.get('force_password_change') and
            request.endpoint not in ['change_password', 'logout'] and
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user_id in WEB_ADMIN_USERS and password == "admin123":
            ...
            # Try with custom password from config
            stored_password = get_admin_password()
            if user_id in WEB_ADMIN_USERS and password == stored_password:
                session['user_id'] = user_id
                return redirect(url_for('index'))
            else:
...
def change_password():
    ...
    if request.method == 'POST':
        ...
        if save_admin_password(new_password):
            session.pop('force_password_change', None)  # Remove force change flag
            return render_template('change_password.html',
